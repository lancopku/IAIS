...
def itm_eval(score_matrix, txt_ids, img_ids, txt2img, img2txts):
    ...
    _, rank_txt = score_matrix.topk(10, dim=1)  # rank_txt: [txt_len, 10]
    ...
    _, rank_img = score_matrix.topk(10, dim=0)  # rank_img: [10, img_len]
...
@torch.no_grad()
def evaluate(model, eval_loader, IAIS=False):
    st = time()
    LOGGER.info("start running Image/Text Retrieval evaluation ...")
    score_matrix = inference(model, eval_loader, IAIS=IAIS)
    ...
    eval_log = itm_eval(all_score, all_txt_ids, all_img_ids, dset.txt2img, dset.img2txts)
...
@torch.no_grad()
def inference(model, eval_loader, IAIS):
    model.eval()
    if hvd.rank() == 0:
    ...
    score_matrix = torch.zeros(len(eval_loader.dataset),
                               ...
                               dtype=torch.float16)  # [txt_len, img_len], note that one img corr.to 5 txt
    for i, mini_batches in enumerate(eval_loader):
        j = 0
        for batch in mini_batches:
            scores = model(batch, compute_loss=False, IAIS=IAIS)  # the scores indicate the matching extend of the i-th txt and the bs imgs
            bs = scores.size(0)
